...
@TeleOp(name="EZ Drive", group="Linear Opmode")

public class EZDrive extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            double grabberOpen = gamepad1.left_trigger;
            double grabberClose = gamepad1.right_trigger;

            if(liftUp || liftDown && (!(liftMotor.getCurrentPosition() > MAX_LIFT_POS) || !(liftMotor.getCurrentPosition() < MIN_LIFT_POS))){
                if(liftUp|| liftDown){
                    if(liftUp && liftMotor.getCurrentPosition() < MAX_LIFT_POS){
                        liftMotor.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
                        liftMotor.setPower(1);
                    }else if(liftDown && liftMotor.getCurrentPosition() > MIN_LIFT_POS){
